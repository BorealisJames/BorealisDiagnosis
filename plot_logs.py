...
def generate_logs():
    ...
    cpu_file_name = "cpu_strength_log.csv"
    ram_file_name = "ram_usage_log.csv"
    wifi_file_name = "wifi_signal_strength_log.csv"
    drone_number = os.getenv('DRONE_NUMBER')

    if drone_number == "1":
        ping_file_name = "ping_uav1_to_uav2_log.csv"
    if drone_number == "2":
        ping_file_name = "ping_uav2_to_uav1_log.csv"

    cpu_df = pd.read_csv(whereami + newest_directory + cpu_file_name, sep= ",")
    ram_df = pd.read_csv(whereami + newest_directory + ram_file_name, sep= ",")
    ping_df = pd.read_csv(whereami + newest_directory + ping_file_name, sep= ",")
    wifi_df = pd.read_csv(whereami + newest_directory + wifi_file_name, sep= ",")
